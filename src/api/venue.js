...
export const postCreateVenue = async (mutationData) => {
  ...
  const res = await axios.post(`${process.env.REACT_APP_API_BASE}/venue`, mutationData, header);
...
export const getAllVenuesByUserId = async (userId) => {
  ...
  const res = await axios.get(`${process.env.REACT_APP_API_BASE}/user/${userId}/venue`, header);
