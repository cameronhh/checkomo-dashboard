...
export const getAllVisitsInRange = async (venueId, minDateTime, maxDateTime) => {
  const header = authHeader(state.token);
...
export const postVisit = async (mutationData) => {
  ...
  const inDateTime = mutationData.customerCheckInDate
    ? parseManualCheckInDatetime(mutationData.customerCheckInDate, mutationData.customerCheckInTime)
