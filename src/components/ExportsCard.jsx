...
const formatExportData = (data) => {
  return data
    .sort((a, b) => moment(b.in_dttm) - moment(a.in_dttm))
    .map((data) => ({
...
export const ExportsCard = ({ venueId, ...rest }) => {
  ...
  const rangedVisitsQuery = useQuery(
    ...
    getAllVisitsInRange,
    {
      enabled: selectedVenue.id && exportRange.in_dttm_min && exportRange.in_dttm_max,
    }
  );
    ...
    <Card>
      ...
      <DateInput
        ...
        onChange={(nextValue) => {
          ...
          setExportRange({
            in_dttm_min: moment.utc(nextValue.value).format('YYYY-MM-DD HH:mm:ss'),
            in_dttm_max: moment.utc(nextValue.value).add(1, 'day').format('YYYY-MM-DD HH:mm:ss'),
          });
        }}
        name="exportDate"
        format="dd/mm/yyyy"
      ...
      <Box align="center" gap="medium">
        {rangedVisitsQuery.data && !rangedVisitsQuery.data.length && (
          <Box pad="small">
        ...
        {rangeDownloadReady ? (
          <CSVLink
            ...
            filename={`CheckInExport ${moment.utc(dateValue).local().format('DD-MM-YYYY')}.csv`}
            onClick={() => {
              return true;
            }}
          >
      ...
      <Box align="center" gap="medium">
        ...
        {allDownloadReady ? (
          ...
          >
            <Button primary label="Download All" onClick={(event) => {}} />
          </CSVLink>
        ) : null}
      </Box>
    </Card>
